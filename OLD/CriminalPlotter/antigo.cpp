...
const int giros_subida = 15;
const int passoDebug = 100;
...
void desenhar()
{
  ...
  while (Serial.available() >= 4)
  {
    ...
    else if (yAtual > yPython){
      
      //Serial.println("Baixo");
      horario = true; //  
      mover(passo * (yAtual - yPython), motorY); // em x
...
void canetar(int giros)
{
  ...
  horario = false;
  mover(giros_subida, motorZ); // Sobe
...
void inicializarMotores()
{
  ...
  //Serial.println("MOTOR Z");
  horario = false;
  mover(giros_subida, motorZ); // Subida
  delay(tmp);
}
...
void debugar(){
  horario = true;              // Esquerda
  mover(passoDebug, motorX); // Desce
  delay(tmp);
  
  horario = false;              // Esquerda
  mover(passoDebug, motorX); // Desce
  delay(tmp);
  ...
  //Serial.println("MOTOR Z");
  horario = true;
  mover(passoDebug, motorZ); // Descida
  delay(tmp);

  // Motor Z
  //Serial.println("MOTOR Z");
  horario = false;
  mover(passoDebug, motorZ); // Subida
  delay(tmp);
  horario = true;
  mover(passoDebug, motorZ); // Subida
  delay(tmp);
